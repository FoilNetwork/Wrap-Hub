...
def found_buys(db, buys, addr=None):
    ...
    for rec in db(
            (no_addr or db.buys.address == addr)
    ...
    "height":-1
        ...
        for r in lUnsp:
            ...
            for income in trans_details:
                ...
                # CopperLark тут нету аккаунта и нет адреса
                # а у Litecoin есть сразу в записи unspent
                ##pay_in = db(db.deal_acc_addrs.address = address).select().first()
                pays.append([
...
# address если не задан то надо для всех искать свои дела и прочее
...
def found_pay_ins_process(db, addr, pays, deal_acc_view=None):
    ...
    for pay in db((not privat or (addr != None and len(addr) > 29))
                  ...
                  & (not addr or db.deal_acc_addrs.address == addr)
                  ##& (not address or db.deal_acc_addrs.xcurr_id == xcurr_in.id)
...
def found_pay_ins(db, addr, pays):
    ...
    if addr:
        ...
        recs = db(
            (db.deal_acc_addrs.address == addr)
            & (db.pay_ins.ref_ == db.deal_acc_addrs.id)
